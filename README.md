...
| <a name="input_cluster_endpoint_public_access"></a> [cluster\_endpoint\_public\_access](#input\_cluster\_endpoint\_public\_access) | GCP Public CIDRs access enabled. This is kept true initially so that other Truefoundry terraform modules which run on user local can access the api server endpoint. | `bool` | `true` | no |
...
| <a name="input_kubernetes_version"></a> [kubernetes\_version](#input\_kubernetes\_version) | Version of GKE | `string` | `"1.33"` | no |
| <a name="input_logging_config"></a> [logging\_config](#input\_logging\_config) | Logging config | <pre>object({<br/>    enable_components = optional(list(string), ["SYSTEM_COMPONENTS", "APISERVER", "CONTROLLER_MANAGER", "SCHEDULER"])<br/>  })</pre> | <pre>{<br/>  "enable_components": [<br/>    "SYSTEM_COMPONENTS",<br/>    "APISERVER",<br/>    "CONTROLLER_MANAGER",<br/>    "SCHEDULER"<br/>  ]<br/>}</pre> | no |
